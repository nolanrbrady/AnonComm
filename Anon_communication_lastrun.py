...
text_20 = visual.TextStim(win=win, name='text_20',
    text="Before we start the experiment we are going to ask you describe a couple of things. You will have one minute to write your response on your phone. There is no need to send this to anyone.\n\nYou will have 60 seconds to write your response, but if you finish early please press spacebar.\n\nOnce you're ready to start please press spacebar.",
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
text_21 = visual.TextStim(win=win, name='text_21',
    text='You have one minute to write your response to the prompt.\n\nIf you finish early please press the spacebar.',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if end_main_intro.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > end_main_intro.tStartRefresh + 60-frameTolerance:
            # keep track of stop time/frame for later
            end_main_intro.tStop = t  # not accounting for scr refresh
            end_main_intro.frameNStop = frameN  # exact frame index
            win.timeOnFlip(end_main_intro, 'tStopRefresh')  # time at next scr refresh
            end_main_intro.status = FINISHED
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if text_13.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > text_13.tStartRefresh + 60-frameTolerance:
            # keep track of stop time/frame for later
    ...
    if skip.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > skip.tStartRefresh + 30-frameTolerance:
            # keep track of stop time/frame for later
    ...
    if text_22.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > text_22.tStartRefresh + 30-frameTolerance:
            # keep track of stop time/frame for later
